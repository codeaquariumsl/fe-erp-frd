...
import { grnScheduleApi, grnApi } from "@/lib/api"
...
export default function BatchSchedulePage() {
    ...
    const [itemRows, setItemRows] = useState<any[]>([]); // [{ itemId, grnId, remarks }]
    ...
    useEffect(() => {
        if (grnApi && grnApi.getItemGrnAvailability) {
            grnApi.getItemGrnAvailability()
                .then((data) => {
                    setItems(data);
                    setItemRows(data.map((row: any) => ({ itemId: row.item.id, grnId: "", remarks: "" })));
                })
                .catch(() => {
                    setItems([]);
                    setItemRows([]);
    ...
    async function handleCreateSchedule(e: React.FormEvent) {
        ...
        const selectedItems = itemRows.filter(row => row.grnId);
        if (selectedItems.length === 0) {
            errors.items = "Please select at least one GRN for scheduling.";
        }
        ...
        for (let i = 0; i < itemRows.length; i++) {
            ...
            if (!row.grnId) continue; // skip if no GRN selected
            
            try {
                await grnScheduleApi.create({
                ...
                errorCount++;
                errorMessages.push(`${items.find(item => item.item.id === row.itemId)?.item?.name || 'Item'}: ${err.message}`);
            }
        }
        ...
        if (successCount > 0) {
            ...
            setTimeout(() => {
                ...
                setItemRows(items.map((row: any) => ({ itemId: row.item.id, grnId: "", remarks: "" })));
                setCreateSuccess("");
            }, 2000);
    ...
    const getSelectedItemsCount = () => {
        return itemRows.filter(row => row.grnId).length;
    };
            ...
            <div className="space-y-2">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-muted-foreground">View and manage batch schedules for GRN and inventory operations.</p>
                    </div>
                </div>
                    ...
                    <Dialog open={openDialog} onOpenChange={setOpenDialog}>
                        ...
                        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
                            <DialogHeader>
                                ...
                                <DialogDescription>
                                    Schedule items for batch processing. Select a date and choose GRNs for each item.
                                </DialogDescription>
                            </DialogHeader>
                            ...
                            <form onSubmit={handleCreateSchedule} className="space-y-6">
                                ...
                                {/* Items Table */}
                                <div className="space-y-2">
                                    <Label className="text-sm font-medium">Item Selection & GRN Assignment</Label>
                                    <div className="border rounded-lg overflow-hidden">
                                        <Table>
                                            <TableHeader>
                                                <TableRow className="bg-muted/50">
                                                    <TableHead className="w-[40%]">Item Details</TableHead>
                                                    <TableHead className="w-[35%]">GRN Selection</TableHead>
                                                        ...
                                                        <TableCell className="font-medium">
                                                            <div className="space-y-1">
                                                                ...
                                                                <div className="text-xs text-muted-foreground">
                                                                    {row.grns?.length || 0} GRN(s) available
                                                                </div>
                                                            </div>
                                                        </TableCell>
                                                        <TableCell>
                                                            <Select
                                                                value={itemRows[idx]?.grnId || ""}
                                                                onValueChange={(val: string) => {
                                                                    const newRows = [...itemRows];
                                                                    newRows[idx].grnId = val;
                                                                    setItemRows(newRows);
                                                            ...
                                                            >
                                                                <SelectTrigger className={cn(
                                                                    ...
                                                                    <SelectValue placeholder={
                                                                        row.grns?.length ? "Select GRN" : "No GRNs available"
                                                                    } />
                                                                </SelectTrigger>
                                                                <SelectContent>
                                    ...
                                    <Table>
                                        <TableHeader>
                                            <TableRow className="bg-muted/50">
                                                ...
                                                <TableHead>Batch/GRN Number</TableHead>
                                                    ...
                                                    <TableCell>
                                                        <div className="space-y-1">
                                                            <div className="font-medium">
                                                                {batch.itemName || (batch.Item ? `${batch.Item.color} ${batch.Item.name} ${batch.Item.country}` : 'Unknown Item')}
                                                            </div>
                                                            {batch.remarks && (
                                                                <div className="text-xs text-muted-foreground">
                                                                    {batch.remarks}
