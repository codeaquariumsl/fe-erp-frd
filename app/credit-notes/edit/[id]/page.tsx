...
import { creditNotesApi, customersApi, itemsApi, Customer, Item, CreditNoteItem, UpdateCreditNoteRequest, CreditNote } from "@/lib/api"
...
export default function EditCreditNotePage() {
    ...
    const [existingCreditNote, setExistingCreditNote] = useState<CreditNote | null>(null)
    ...
    const loadData = async () => {
        try {
            const [customersData, itemsData, creditNoteData] = await Promise.all([
                customersApi.getAll(),
                itemsApi.getAll(),
                creditNotesApi.getById(parseInt(id))
            ])
            setCustomers(Array.isArray(customersData) ? customersData : [])
            setItems(Array.isArray(itemsData) ? itemsData : [])

            if (creditNoteData) {
                setExistingCreditNote(creditNoteData)
                setFormData({
                    customerId: creditNoteData.customerId.toString(),
                    invoiceId: creditNoteData.invoiceId ? creditNoteData.invoiceId.toString() : "",
                    creditNoteDate: new Date(creditNoteData.creditNoteDate).toISOString().split("T")[0],
                    reason: creditNoteData.reason || "",
                    isTaxCreditNote: creditNoteData.isTaxCreditNote,
                    taxRate: creditNoteData.taxRate.toString(),
                    notes: creditNoteData.notes || "",
                })

                const items = creditNoteData.CreditNoteItems || creditNoteData.items || []
                setCreditNoteItems(items.map(item => ({
                    itemId: item.itemId,
                    code: item.code,
                    qty: Number(item.qty),
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const requestData: UpdateCreditNoteRequest = {
            ...
            await creditNotesApi.update(parseInt(id), requestData)
            toastr.success("Credit note updated successfully")
            router.push("/credit-notes")
        } catch (error: any) {
            console.error("Error updating credit note:", error)
            toastr.error(error.message || "Failed to update credit note")
        } finally {
            setLoading(false)
        }
    }
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex justify-end gap-4">
                        ...
                        <Button type="submit" disabled={loading}>
                            {loading ? "Updating..." : "Update Credit Note"}
                        </Button>
                    </div>
                </form>
            </div>
